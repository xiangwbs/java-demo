...
public class ExcelUtil
{
	 ...
	 * @return
	 */
	private static List<String> exportListFromExcel(File file, int sheetNum)
			throws IOException
	{
	 ...
	 * @throws IOException
	 */
	private static List<String> exportListFromExcel(InputStream is,
			...
			int sheetNum)
	{
		...
		for (int rowIx = minRowIx; rowIx <= maxRowIx; rowIx++)
		{
			...
			list.add(sb.substring(1, sb.length()).toString());
		}
		return list;
	}
	...
	public static <T> List<T> getObjectFromExcel(File file, int sheetNum,
			Class<T> classVO)
	{
			...
			if (CollectionUtils.isNotEmpty(importObjects))
			{
				...
				for (int i = 1; i < importObjects.size(); i++)
				{
					...
					for (int j = 0; j < values.length; j++)
					{
						...
						try
						{
							setMethod = classVO.getMethod(setMethodStr,
									String.class);
						...
						catch (Exception e)
						{
							try
							{
								setMethod = classVO.getMethod(setMethodStr,
										Integer.class);
								setMethod.invoke(obj, Double.valueOf(values[j])
										.intValue());
							}
							catch (Exception e1)
							{
	...
	public static <T> List<String[]> convert2List(List<T> list) 
	{
		...
		Class<? extends Object> classOfT = obj4Class.getClass();
